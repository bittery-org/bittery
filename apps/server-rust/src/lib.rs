...
pub use auth::{
	create_auth_router, rpc_request_context_middleware, SeededSession, SessionService,
};
...
pub async fn privateData(ctx: auth::RefreshSessionContext) -> PrivateDataResponse {
	PrivateDataResponse {
		...
		user: PrivateDataUserResponse {
			...
			expires_at: ctx.session.expires_at.format(&time::format_description::well_known::Rfc3339).unwrap_or_else(|_| ctx.session.expires_at.unix_timestamp().to_string()),
