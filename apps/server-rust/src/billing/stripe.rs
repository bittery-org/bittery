...
pub async fn create_checkout_session(
	...
	let mut form = vec![
		...
		("line_items[0][price]".to_string(), input.price_id.to_string()),
		(
...
pub async fn preview_upcoming_team_seat_invoice(
	...
	let subscription_item = stripe_subscription_item_id
		.and_then(|item_id| subscription.items.data.iter().find(|item| item.id == item_id))
	...
	let mut lines = source_lines
		.into_iter()
		.filter(|line| line.amount != 0)
		.map(|line| -> Result<TeamSeatInvoicePreviewLine, StripeClientError> {
			let period_start = OffsetDateTime::from_unix_timestamp(line.period.start)
				.map_err(|_| StripeClientError::InvalidResponse("Invalid Stripe period start".to_string()))?;
			let period_end = OffsetDateTime::from_unix_timestamp(line.period.end)
				.map_err(|_| StripeClientError::InvalidResponse("Invalid Stripe period end".to_string()))?;
...
fn get_line_unit_amount_cents(line: &StripeInvoiceLine) -> Option<i64> {
	...
	match line.quantity {
		Some(quantity) if quantity > 0 => Some((line.amount as f64 / quantity as f64).round() as i64),
		_ => None,
	}
}
...
async fn post_form<T: DeserializeOwned>(
	...
	let response = http_client()
		...
		.send()
		.await
		.map_err(|error| StripeClientError::RequestFailed(format!("Stripe request failed: {error}")))?;
...
async fn parse_response<T: DeserializeOwned>(
	...
	let body = response
		.text()
		.await
		.map_err(|error| StripeClientError::RequestFailed(format!("Failed to read Stripe response: {error}")))?;
