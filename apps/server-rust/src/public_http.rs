...
async fn favicon(Path(domain): Path<String>, State(app_state): State<AppState>) -> Response {
	let Some(domain) = normalize_favicon_domain(&domain) else {
		return (StatusCode::NOT_FOUND, cache_control_header(FAVICON_CACHE_CONTROL)).into_response();
	};
	...
	match get_fetched_favicon(pool, &domain).await {
		Ok(Some(icon)) => (
			...
			.into_response(),
		Ok(None) => (StatusCode::NOT_FOUND, cache_control_header(FAVICON_CACHE_CONTROL)).into_response(),
		Err(error) => {
			warn!(?error, domain, "failed to reload fetched favicon from database");
...
async fn stripe_webhook(
	...
	if !is_stripe_webhook_configured() {
		return (StatusCode::SERVICE_UNAVAILABLE, "Stripe webhook not configured").into_response();
	}

	let Some(pool) = app_state.db_pool.as_ref() else {
		return (StatusCode::SERVICE_UNAVAILABLE, "Database is not configured").into_response();
	};
