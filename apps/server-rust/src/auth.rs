...
use qubit::{
	...
	handler,
	server::{ErrorCode, Extensions, FromRequestExtensions, RpcError, Router},
};
...
impl SessionService {
	...
	pub async fn delete_other_user_sessions(
		...
		match &self.backend {
			SessionBackend::Memory(inner) => {
				Ok(delete_other_memory_sessions(inner, user_id, current_session_id))
			}
			SessionBackend::Postgres(store) => {
				store.delete_other_user_sessions(user_id, current_session_id).await
			}
		}
	}
	...
	async fn get_owned_session(
		...
		match &self.backend {
			SessionBackend::Memory(inner) => Ok(get_owned_memory_session(inner, session_id, user_id)),
	...
	#[cfg(test)]
	async fn issue_session_for_tests(
		&self,
		user_id: &str,
		platform: &str,
		...
		match &self.backend {
			SessionBackend::Memory(inner) => issue_memory_session_for_tests(
				inner,
				user_id,
				platform,
				client_id,
			),
			SessionBackend::Postgres(_) => {
				panic!("test session issuance is only supported for the memory backend")
			}
		}
	}
}
...
impl PostgresSessionStore {
	async fn create_session(
		...
		if platform == "web" && client_id.is_some() {
		...
		.bind(device_info.as_ref().and_then(|d| d.browser_version.as_deref()))
	...
	async fn verify_token(&self, token: &str) -> Option<VerifiedSession> {
		...
		session.map(|row| VerifiedSession {
			...
			client_id: normalized_session_client_id(
	...
	async fn refresh_session(
		...
		let client_id = normalized_session_client_id(current.platform.as_deref(), current.client_id);
		...
		let device_name = if platform == "web" && client_id.is_some() {
	...
	async fn delete_other_user_sessions(
		...
		query_scalar::<_, String>("DELETE FROM session WHERE user_id = $1 AND id != $2 RETURNING id")
			.bind(user_id)
	...
	async fn list_devices(
		...
		let sessions = rows.into_iter().map(snapshot_from_db_session).collect::<Vec<_>>();
	...
	async fn revoke_device(
		...
		if is_grouped_web_session(&existing_session) {
...
fn touch_memory_session(inner: &Arc<SessionServiceInner>, session_id: &str) {
	let mut sessions = inner
		...
		.write()
		.expect("memory session store poisoned");
	if let Some(record) = sessions.values_mut().find(|record| record.session_id == session_id) {
...
pub async fn requestSignupVerification(
	...
	if let Some(invitation_token) = input.invitation_token.as_deref() {
		let _ = get_pending_invitation_for_signup(pool, invitation_token, &normalized_email).await?;
	} else if bittery_mode() == "self-hosted" && has_any_registered_user(pool).await? {
		return Err(AuthRpcError {
			code: "FORBIDDEN".to_string(),
			message: "Public registration is disabled. Ask an admin for an invite link.".to_string(),
		});
	}

	let code = create_signup_verification(pool, &normalized_email, input.invitation_token.as_deref())
		.await?;
...
pub async fn signup(
	...
	let team_name = signup_team_name(self_hosted_mode, team_type, input.organization_name.as_deref());
	let member_limit = if self_hosted_mode {
		None
	} else {
		plan_member_limit(selected_plan)
	};
	let billing_status = if selected_plan == "free" { "none" } else { "incomplete" };
	let user_id = input.user_id.clone().unwrap_or_else(|| generate_resource_id("user"));
	let team_id = generate_resource_id("team");
	let vault_id = input.vault_id.clone().unwrap_or_else(|| generate_resource_id("vault"));

	let mut transaction = pool
		.begin()
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to start signup transaction"); internal_handler_error("Failed to start signup transaction") })?;

	insert_user_account(&mut transaction, CreateUserParams {
		user_id: &user_id,
	...
	query("UPDATE \"user\" SET team_id = $1, role = 'owner' WHERE id = $2")
		...
		.execute(transaction.as_mut())
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to link user to team"); internal_handler_error("Failed to link user to team") })?;
	insert_personal_vault(&mut transaction, &vault_id, &user_id, &input.encrypted_vault_key).await?;

	transaction
		.commit()
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to commit signup"); internal_handler_error("Failed to commit signup") })?;

	let session = ctx.app_state.sessions.create_session(&user_id, &ctx.request).await?;
...
pub async fn signupWithInvitation(
	...
	if let Some(member_limit) = invitation.member_limit {
		let current_members = query_scalar::<_, i64>(
			"SELECT COUNT(*)::bigint FROM \"user\" WHERE team_id = $1",
		)
		.bind(&invitation.team_id)
		.fetch_one(pool)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to load team members"); internal_handler_error("Failed to load team members") })?;
	...
	let user_id = input.user_id.clone().unwrap_or_else(|| generate_resource_id("user"));
	let personal_vault_id = input.vault_id.clone().unwrap_or_else(|| generate_resource_id("vault"));
	let vault_role = if invitation.role == "admin" { "admin" } else { "member" };

	let mut transaction = pool
		.begin()
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to start invited signup transaction"); internal_handler_error("Failed to start invited signup transaction") })?;
	...
	.await?;
	query("UPDATE \"user\" SET team_id = $1, role = $2 WHERE id = $3")
		.bind(&invitation.team_id)
		.bind(&invitation.role)
		.bind(&user_id)
		.execute(transaction.as_mut())
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to link invited user to team"); internal_handler_error("Failed to link invited user to team") })?;
	...
	for pending_key in &pending_keys {
		query(
			"INSERT INTO vault_key (id, vault_id, user_id, encrypted_vault_key, role) VALUES ($1, $2, $3, $4, $5)",
		)
	...
	query("UPDATE team_invitation SET status = 'accepted', accepted_at = $1 WHERE id = $2")
		...
		.execute(transaction.as_mut())
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to accept invitation"); internal_handler_error("Failed to accept invitation") })?;

	transaction
		.commit()
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to commit invited signup"); internal_handler_error("Failed to commit invited signup") })?;
...
pub async fn startLogin(
	...
	query("DELETE FROM login_attempt WHERE expires_at <= $1")
		...
		.execute(pool)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to prune login attempts"); internal_handler_error("Failed to prune login attempts") })?;
	...
	let ephemeral = server
		...
		.map_err(|e| { tracing::error!(error = %e, "Failed to create login challenge"); internal_handler_error("Failed to create login challenge") })?;
...
pub async fn finishLogin(
	...
	let session = ctx.app_state.sessions.create_session(&user.id, &ctx.request).await?;
...
pub async fn resetPassword(
	...
	let recovery_email = verify_recovery_token(&input.recovery_token)
		.await
		.ok_or_else(|| unauthorized_handler_error("Invalid recovery session"))?;
	let (user_id, revoked_session_ids) = reset_user_password_with_recovery(pool, &recovery_email, &input)
		.await
		.map_err(|_| unauthorized_handler_error("Invalid recovery session"))?;
	record_session_revocations(pool, &user_id, &revoked_session_ids, "password_reset_via_recovery")
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to record session revocations"); internal_handler_error("Failed to record session revocations") })?;
	let session = ctx.app_state.sessions.create_session(&user_id, &ctx.request).await?;
...
pub async fn updateEmail(
	...
	let existing_user_id = query_scalar::<_, String>(
		"SELECT id FROM \"user\" WHERE LOWER(email) = $1 LIMIT 1",
	)
	.bind(&normalized_new_email)
	.fetch_optional(pool)
	.await
	.map_err(|e| { tracing::error!(error = %e, "Failed to check email"); internal_handler_error("Failed to check email") })?;
	if existing_user_id.as_deref().is_some_and(|value| value != ctx.session.user_id) {
	...
	let revoked_session_ids = ctx.app_state
		.sessions
		.delete_all_user_sessions(&ctx.session.user_id)
		.await?;
	record_session_revocations(pool, &ctx.session.user_id, &revoked_session_ids, "email_changed")
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to record session revocations"); internal_handler_error("Failed to record session revocations") })?;
	query(
...
pub async fn changePassword(
	...
	let revoked_session_ids = ctx.app_state
		...
		.delete_all_user_sessions(&ctx.session.user_id)
		.await?;
	record_session_revocations(pool, &ctx.session.user_id, &revoked_session_ids, "password_changed")
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to record session revocations"); internal_handler_error("Failed to record session revocations") })?;
	query(
...
pub async fn deleteAccount(
		...
		&& user.team_type.as_deref() != Some("personal")
	{
		if let Some(team_id) = user.team_id.as_deref() {
			let remaining_members = query_scalar::<_, i64>(
				"SELECT COUNT(*)::bigint FROM \"user\" WHERE team_id = $1",
			)
			.bind(team_id)
			.fetch_one(pool)
			...
			let remaining_vaults = query_scalar::<_, i64>(
				"SELECT COUNT(*)::bigint FROM vault WHERE team_id = $1",
			)
			.bind(team_id)
			.fetch_one(pool)
			.await
			.map_err(|e| { tracing::error!(error = %e, "Failed to load team vaults"); internal_handler_error("Failed to load team vaults") })?;
...
pub async fn checkEmail(
	...
	let secret_key_hint = match ctx.app_state.db_pool.as_ref() {
		...
		.fetch_optional(pool)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to load account"); internal_handler_error("Failed to load account") })?
...
pub async fn revokeDevice(
	...
	if let (Some(target_session), Some(current_session)) = (&target_session, &current_session) {
		if is_grouped_web_session(target_session)
			&& is_grouped_web_session(current_session)
	...
	if !revoked_session_ids.is_empty() {
		...
		if let Some(pool) = ctx.app_state.db_pool.as_ref() {
			record_session_revocations(pool, &ctx.session.user_id, &revoked_session_ids, "device_revoked")
				.await
				.map_err(|e| { tracing::error!(error = %e, "Failed to record session revocations"); internal_handler_error("Failed to record session revocations") })?;
			query(
...
pub async fn renameDevice(
	...
	ctx.app_state
		.sessions
		.rename_device(&input.session_id, &ctx.session.user_id, input.device_name.trim())
		.await?;
	Ok(LogoutResponse { success: true })
}
...
pub async fn heartbeat(
	...
	ctx.app_state.sessions.heartbeat(&ctx.session.session_id).await?;
	Ok(LogoutResponse { success: true })
}
...
pub async fn logoutAll(
	...
	if let Some(pool) = ctx.app_state.db_pool.as_ref() {
		record_session_revocations(pool, &ctx.session.user_id, &revoked_session_ids, "logout_all")
			.await
			.map_err(|e| { tracing::error!(error = %e, "Failed to record session revocations"); internal_handler_error("Failed to record session revocations") })?;
		query(
			"INSERT INTO audit_log (id, user_id, action, entity_type, entity_id, created_at) VALUES ($1, $2, $3, $4, $5, $6)",
...
fn normalize_session_platform(platform: Option<&str>) -> String {
	match platform.map(|value| value.trim().to_lowercase()) {
		...
		Some(normalized) if normalized == "ios" || normalized == "android" => {
			"mobile".to_string()
		}
		Some(normalized)
			if normalized == "web"
...
fn normalized_session_client_id(
	...
	if normalize_session_platform(platform) != "web" {
		return None;
	}
...
fn build_login_attempt_id(normalized_email_hash: &str) -> String {
	format!("{normalized_email_hash}:{}", generate_resource_id("attempt"))
}
...
fn deterministic_fake_hint(email: &str) -> String {
	let secret = std::env::var("JWT_SECRET")
		.unwrap_or_else(|_| "bittery-dev-email-hint-secret".to_string());
...
fn is_dev_auth_stub_enabled() -> bool {
	matches!(std::env::var("BITTERY_ENABLE_DEV_AUTH_STUBS").ok().as_deref(), Some("true"))
...
fn create_signup_verification_token(
	...
	let expires_at = (now_utc() + Duration::minutes(SIGNUP_VERIFICATION_TTL_MINUTES)).unix_timestamp()
		as usize;
...
fn create_recovery_token(email: &str) -> Result<String, AuthRpcError> {
	let issued_at = now_utc().unix_timestamp() as usize;
	let expires_at =
		(now_utc() + Duration::minutes(RECOVERY_VERIFICATION_TTL_MINUTES)).unix_timestamp()
			as usize;
...
async fn get_pending_invitation_for_signup(
	...
	if !team_management_enabled(&invitation.billing_plan, &invitation.billing_status) {
		return Err(AuthRpcError {
			code: "FORBIDDEN".to_string(),
			message: "This team cannot accept invitations on its current plan or billing status.".to_string(),
		});
	}
...
async fn create_recovery_verification(
	pool: &PgPool,
	email: &str,
) -> Result<String, AuthRpcError> {
	let code = generate_signup_verification_code();
	let now = now_utc();
	query(
		"UPDATE recovery_verification SET used_at = $1 WHERE email = $2 AND used_at IS NULL",
	)
	.bind(now)
	.bind(email)
	.execute(pool)
	.await
	.map_err(|e| { tracing::error!(error = %e, "Failed to update recovery verification"); internal_handler_error("Failed to update recovery verification") })?;
	...
	.execute(pool)
	.await
	.map_err(|e| { tracing::error!(error = %e, "Failed to create recovery verification"); internal_handler_error("Failed to create recovery verification") })?;
...
async fn consume_signup_verification_code(
	...
	if let Some(active) = active {
		...
		query(
			"UPDATE signup_verification SET attempts = $1, used_at = $2, updated_at = $2 WHERE id = $3",
		)
		.bind(next_attempts)
		.bind(used_at)
		.bind(active.id)
...
async fn verify_recovery_code_attempt(
	...
	if let Some(active) = active {
		...
		query(
			"UPDATE recovery_verification SET attempts = $1, used_at = $2 WHERE id = $3",
		)
		.bind(next_attempts)
		.bind(used_at)
		.bind(active.id)
		.execute(pool)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to update recovery verification attempts"); internal_handler_error("Failed to update recovery verification attempts") })?;
	}
...
async fn reset_user_password_with_recovery(
	...
	let mut transaction = pool
		.begin()
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to start recovery reset transaction"); internal_handler_error("Failed to start recovery reset transaction") })?;
	...
	let revoked_session_ids = load_user_session_ids(pool, &user_id)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to load recovery sessions"); internal_handler_error("Failed to load recovery sessions") })?;
	...
	for vault_key in &input.encrypted_vault_keys {
		...
		.execute(transaction.as_mut())
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to update recovery vault keys"); internal_handler_error("Failed to update recovery vault keys") })?;
	}
	...
	query("DELETE FROM session WHERE user_id = $1")
		...
		.execute(transaction.as_mut())
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to revoke sessions after recovery"); internal_handler_error("Failed to revoke sessions after recovery") })?;
	query("UPDATE recovery_verification SET used_at = $1 WHERE id = $2")
		.bind(now)
		.bind(&verification.id)
		.execute(transaction.as_mut())
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to finalize recovery verification"); internal_handler_error("Failed to finalize recovery verification") })?;

	transaction
		.commit()
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to commit recovery reset"); internal_handler_error("Failed to commit recovery reset") })?;
...
async fn update_user_email_data(
	...
	let mut transaction = pool
		.begin()
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to start email update transaction"); internal_handler_error("Failed to start email update transaction") })?;
	query(
	...
	apply_encrypted_vault_key_updates(&mut transaction, user_id, &input.encrypted_vault_keys).await?;
	transaction
		.commit()
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to commit email update"); internal_handler_error("Failed to commit email update") })?;
	Ok(())
}
...
async fn update_user_password_data(
	...
	let mut transaction = pool
		.begin()
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to start password update transaction"); internal_handler_error("Failed to start password update transaction") })?;
	query(
	...
	transaction
		.commit()
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to commit password update"); internal_handler_error("Failed to commit password update") })?;
	Ok(())
}
...
async fn update_user_secret_key_data(
	...
	let mut transaction = pool
		.begin()
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to start secret key transaction"); internal_handler_error("Failed to start secret key transaction") })?;
	query(
	...
	transaction
		.commit()
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to commit secret key update"); internal_handler_error("Failed to commit secret key update") })?;
	Ok(())
}
...
async fn store_recovery_key_data(
	...
	query(
		"UPDATE \"user\" SET encrypted_master_key = $1, recovery_key_hint = $2 WHERE id = $3",
	)
	.bind(&input.encrypted_master_key)
	.bind(&input.recovery_key_hint)
	.bind(user_id)
	.execute(pool)
	.await
	.map_err(|e| { tracing::error!(error = %e, "Failed to store recovery key"); internal_handler_error("Failed to store recovery key") })?;
	Ok(())
}
...
async fn delete_user_account_data(pool: &PgPool, user_id: &str) -> Result<(), AuthRpcError> {
	query("DELETE FROM \"user\" WHERE id = $1")
		...
		.execute(pool)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to delete account"); internal_handler_error("Failed to delete account") })?;
	Ok(())
}
...
async fn apply_encrypted_vault_key_updates(
	...
	for vault_key in encrypted_vault_keys {
		...
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to update vault keys"); internal_handler_error("Failed to update vault keys") })?;
	}
	Ok(())
}
...
async fn ensure_user_does_not_exist(pool: &PgPool, email: &str) -> Result<(), AuthRpcError> {
	let existing = query_scalar::<_, String>("SELECT id FROM \"user\" WHERE LOWER(email) = $1 LIMIT 1")
		.bind(email)
		.fetch_optional(pool)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to check account"); internal_handler_error("Failed to check account") })?;
	if existing.is_some() {
		return Err(AuthRpcError {
			code: "BAD_REQUEST".to_string(),
...
async fn insert_team(
	...
	query(
		"INSERT INTO team (id, name, owner_id, type, member_limit, billing_plan, billing_status) VALUES ($1, $2, $3, $4::team_type, $5, $6, $7)",
	)
	.bind(team_id)
	.bind(team_name)
	.bind(owner_id)
...
fn signup_team_name(
	...
	let provided = organization_name.map(str::trim).filter(|value| !value.is_empty());
	if self_hosted_mode {
...
fn validate_hex_string(value: &str, message: &str) -> Result<(), AuthRpcError> {
	if value.is_empty() || value.len() % 2 != 0 || !value.chars().all(|character| character.is_ascii_hexdigit()) {
...
async fn assert_pending_vault_keys_authorized(
	...
	.fetch_one(pool)
	.await
	.map_err(|e| { tracing::error!(error = %e, "Failed to validate pendingVaultKeys vaults"); internal_handler_error("Failed to validate pendingVaultKeys vaults") })?;
	...
	if authorized_vault_ids.len() != vault_ids.len() {
		return Err(AuthRpcError {
			code: "FORBIDDEN".to_string(),
			message: "You do not have permission to grant access for one or more vaults".to_string(),
		});
	}
	Ok(())
}
...
fn build_device_session_responses(
	...
	let mut grouped_web_sessions = HashMap::<String, Vec<SessionSnapshot>>::new();

	for session in sessions {
		if is_grouped_web_session(&session) {
			let client_id = session.client_id.clone().expect("grouped web session has client id");
			grouped_web_sessions.entry(client_id).or_default().push(session);
		} else {
			logical_sessions.push(session);
		}
	}

	for group in grouped_web_sessions.values() {
...
fn is_grouped_web_session(value: &SessionSnapshot) -> bool {
	value.platform == "web" && value.client_id.is_some()
}
...
async fn has_any_registered_user(pool: &PgPool) -> Result<bool, AuthRpcError> {
	let user_id = query_scalar::<_, String>("SELECT id FROM \"user\" LIMIT 1")
		.fetch_optional(pool)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to load registration status"); internal_handler_error("Failed to load registration status") })?;
	Ok(user_id.is_some())
}
...
fn detect_platform(ua: &str, app_platform: Option<&str>) -> String {
	...
	if ua.contains("iphone")
		|| ua.contains("ipad")
		|| (ua.contains("ios") && ua.contains("mobile"))
	{
...
#[cfg(test)]
mod tests {
	use super::{deterministic_fake_hint, SessionBackend, SessionService};
	use time::{Duration, OffsetDateTime};

	#[tokio::test]
	async fn refresh_rotates_the_previous_session() {
		let sessions = SessionService::default();
		let seeded = sessions.seeded_session().expect("memory backend should seed");
	...
	async fn refresh_preserves_platform_duration_policy() {
		let sessions = SessionService::default();
		let current = sessions.issue_session_for_tests(
			"extension-user",
			"extension",
		...
		let remaining_days =
			(refreshed.expires_at - time::OffsetDateTime::now_utc()).whole_days();
	...
	async fn list_devices_collapses_grouped_web_sessions() {
		...
		assert!(devices.iter().any(|device| device.id == current.session_id && device.is_current_session));
	}
