...
pub async fn teamEvents(
	...
	let member_map = HashMap::<String, TeamMemberRow>::from_iter(
		members.iter().cloned().map(|member| (member.id.clone(), member)),
	);
...
async fn load_team_members(
	...
	query_as::<_, TeamMemberRow>(
		"SELECT id, name, email FROM \"user\" WHERE team_id = $1",
	)
	.bind(team_id)
	.fetch_all(pool)
	.await
...
fn normalize_input(input: &TeamEventsInput) -> Result<NormalizedInput, AuditRpcError> {
	...
	if let (Some(from), Some(to)) = (from, to) {
		if from > to {
			return Err(bad_request_error("The from date must be before the to date"));
		}
	}
...
fn encode_cursor(event: &TeamEvent) -> String {
	...
	URL_SAFE_NO_PAD.encode(serde_json::to_vec(&payload).expect("cursor serialization should succeed"))
}
...
fn to_audit_event(row: AuditEventRow, member_map: &HashMap<String, TeamMemberRow>) -> TeamEvent {
	...
	TeamEvent {
		id: row.id,
		timestamp: row.created_at.format(&Rfc3339).expect("timestamp should format"),
...
fn to_share_access_event(
	...
	TeamEvent {
		id: row.id,
		timestamp: row.accessed_at.format(&Rfc3339).expect("timestamp should format"),
