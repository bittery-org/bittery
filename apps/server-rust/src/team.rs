...
pub async fn pending(
	...
	.fetch_optional(pool)
	.await
	.map_err(|e| { tracing::error!(error = %e, "Failed to load user"); internal_error("Failed to load user") })?
...
pub async fn vaults(
	...
	.fetch_all(pool)
	.await
	.map_err(|e| { tracing::error!(error = %e, "Failed to load team vaults"); internal_error("Failed to load team vaults") })?;
...
pub async fn update(
	...
	let current_user = current_user.ok_or_else(|| forbidden_error("You are not a member of this team"))?;
	...
	match (input.name.as_ref(), input.image_key.as_ref()) {
		(Some(name), Some(image_key)) => {
			query("UPDATE team SET name = $1, image_key = $2, updated_at = $3 WHERE id = $4")
				...
				.execute(pool)
				.await
				.map_err(|e| { tracing::error!(error = %e, "Failed to update team"); internal_error("Failed to update team") })?;
		}
		(Some(name), None) => {
...
pub async fn delete(
	...
	let mut transaction = pool
		.begin()
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to start transaction"); internal_error("Failed to start transaction") })?;
	...
	let member_count = query_scalar::<_, i64>(
		"SELECT COUNT(*)::bigint FROM \"user\" WHERE team_id = $1",
	)
	.bind(&input.team_id)
	.fetch_one(&mut *transaction)
	.await
	.map_err(|e| { tracing::error!(error = %e, "Failed to count team members"); internal_error("Failed to count team members") })?;
	if member_count != 1 {
		return Err(bad_request_error(
	...
	.fetch_one(&mut *transaction)
	.await
	.map_err(|e| { tracing::error!(error = %e, "Failed to count team vaults"); internal_error("Failed to count team vaults") })?;
	...
	query("DELETE FROM team_invitation WHERE team_id = $1")
		...
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to delete team invitations"); internal_error("Failed to delete team invitations") })?;
	query("DELETE FROM team WHERE id = $1")
		.bind(&input.team_id)
		.execute(&mut *transaction)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to delete team"); internal_error("Failed to delete team") })?;

	transaction
		.commit()
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to commit team deletion"); internal_error("Failed to commit team deletion") })?;
...
pub async fn leave(
	...
	let team_vaults = load_team_vaults_with_user_access(pool, &input.team_id, &ctx.session.user_id)
		.await?;
	...
	let member_actor = load_team_membership_actor(pool, &ctx.session.user_id)
		.await?
		.ok_or_else(|| forbidden_error("You are not a member of this team"))?;
	let billing_plan = member_actor.billing_plan.unwrap_or_else(|| "free".to_string());

	let result = async {
		let mut transaction = pool
			.begin()
			.await
			.map_err(|e| { tracing::error!(error = %e, "Failed to start leave-team transaction"); internal_error("Failed to start leave-team transaction") })?;
		...
		transaction
			.commit()
			.await
			.map_err(|e| { tracing::error!(error = %e, "Failed to commit leave-team transaction"); internal_error("Failed to commit leave-team transaction") })?;
		Ok::<(), TeamRpcError>(())
	}
	.await;
	...
	let revoked_session_ids = load_user_session_ids(pool, &ctx.session.user_id)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to load sessions after leaving team"); internal_error("Failed to load sessions after leaving team") })?;
	query("DELETE FROM session WHERE user_id = $1")
		.bind(&ctx.session.user_id)
		.execute(pool)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to revoke sessions after leaving team"); internal_error("Failed to revoke sessions after leaving team") })?;
	record_session_revocations(pool, &ctx.session.user_id, &revoked_session_ids, "team_left")
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to record session revocations after leaving team"); internal_error("Failed to record session revocations after leaving team") })?;
...
pub async fn getLeaveRotationData(
	...
	let team_vaults = load_team_vaults_with_user_access(pool, &input.team_id, &ctx.session.user_id)
		.await?;
	let rotation_vaults =
		load_rotation_vault_data(pool, team_vaults, &ctx.session.user_id).await?;
...
pub async fn send(
	...
	.fetch_optional(pool)
	.await
	.map_err(|e| { tracing::error!(error = %e, "Failed to load existing user"); internal_error("Failed to load existing user") })?;
	if existing_user
		.as_ref()
		.and_then(|value| value.team_id.as_ref())
		.is_some()
	...
	let serialized_pending_vault_keys = if pending_vault_keys.is_empty() {
		...
		Some(
			serde_json::to_string(&pending_vault_keys)
				.map_err(|e| { tracing::error!(error = %e, "Failed to serialize pendingVaultKeys"); internal_error("Failed to serialize pendingVaultKeys") })?,
		)
	};

	query(
		"INSERT INTO team_invitation (id, team_id, email, role, invited_by_id, token, pending_vault_keys, expires_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
	)
	.bind(&invitation_id)
	.bind(&input.team_id)
	.bind(&input.email)
...
pub async fn accept(
	...
	if invitation.expires_at < OffsetDateTime::now_utc() {
		query("UPDATE team_invitation SET status = 'expired' WHERE id = $1")
			...
			.execute(pool)
			.await
			.map_err(|e| { tracing::error!(error = %e, "Failed to expire invitation"); internal_error("Failed to expire invitation") })?;
	...
	query("UPDATE \"user\" SET team_id = $1, role = $2 WHERE id = $3")
		...
		.execute(&mut *transaction)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to update team membership"); internal_error("Failed to update team membership") })?;
	...
	for pending_key in pending_keys {
		...
		.fetch_one(&mut *transaction)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to load existing vault access"); internal_error("Failed to load existing vault access") })?;
	...
	query("UPDATE team_invitation SET status = 'accepted', accepted_at = $1 WHERE id = $2")
		...
		.execute(&mut *transaction)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to accept invitation"); internal_error("Failed to accept invitation") })?;

	transaction
		.commit()
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to commit invitation acceptance"); internal_error("Failed to commit invitation acceptance") })?;
...
pub async fn decline(
	...
	query("UPDATE team_invitation SET status = 'declined' WHERE id = $1")
		...
		.execute(pool)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to decline invitation"); internal_error("Failed to decline invitation") })?;
...
pub async fn cancel(
	...
	query("DELETE FROM team_invitation WHERE id = $1")
		...
		.execute(pool)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to cancel invitation"); internal_error("Failed to cancel invitation") })?;
...
pub async fn resend(
	...
	query("UPDATE team_invitation SET expires_at = $1, status = 'pending' WHERE id = $2")
		...
		.execute(pool)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to resend invitation"); internal_error("Failed to resend invitation") })?;
...
mod member_handlers {
	...
	pub async fn getTeamRotationData(
		...
		if !matches!(actor.role.as_str(), "owner" | "admin") {
			return Err(forbidden_error("Only owner or admin can perform key rotation"));
		}
		assert_optional_team_management_entitlement(
			actor.billing_plan.as_deref(),
			actor.billing_status.as_deref(),
		...
		let rotation_vaults = load_rotation_vault_data(
			pool,
			removal_scope.removable_vaults,
			&input.exclude_user_id,
		)
		.await?;
		Ok(RotationDataResponse {
	...
	pub async fn remove(
		...
		if ctx.session.user_id == input.user_id {
			return Err(bad_request_error("You cannot remove yourself from the team"));
		}
		...
		let target_user_name = query_scalar::<_, String>(
			"SELECT name FROM \"user\" WHERE id = $1 LIMIT 1",
		)
		.bind(&input.user_id)
		.fetch_optional(pool)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to load target user name"); internal_error("Failed to load target user name") })?
		.ok_or_else(|| not_found_error("Team member not found"))?;
		let removal_scope = load_team_removal_scope(
			pool,
			&input.team_id,
		...
		let result = async {
			let mut transaction = pool
				.begin()
				.await
				.map_err(|e| { tracing::error!(error = %e, "Failed to start team member removal transaction"); internal_error("Failed to start team member removal transaction") })?;
			apply_team_vault_rotations(
				&mut transaction,
				&input.vault_rotations,
				&rotation_record_map,
			...
			transaction
				.commit()
				.await
				.map_err(|e| { tracing::error!(error = %e, "Failed to commit team member removal"); internal_error("Failed to commit team member removal") })?;
			Ok::<(), TeamRpcError>(())
		}
		.await;
		if let Err(error) = result {
		...
		let revoked_session_ids = load_user_session_ids(pool, &input.user_id)
			.await
			.map_err(|e| { tracing::error!(error = %e, "Failed to load removed user sessions"); internal_error("Failed to load removed user sessions") })?;
		query("DELETE FROM session WHERE user_id = $1")
			.bind(&input.user_id)
			.execute(pool)
			.await
			.map_err(|e| { tracing::error!(error = %e, "Failed to revoke removed user sessions"); internal_error("Failed to revoke removed user sessions") })?;
		record_session_revocations(pool, &input.user_id, &revoked_session_ids, "team_member_removed")
			.await
			.map_err(|e| { tracing::error!(error = %e, "Failed to record removed user session revocations"); internal_error("Failed to record removed user session revocations") })?;
		insert_team_member_audit_log(
			pool,
			&ctx.session.user_id,
			"team_member_removed",
...
fn validate_rotation_vault_inputs(
	...
	for vault_rotation in vault_rotations {
		...
		if vault_rotation.key_rotation.re_encrypted_items.len()
			> MAX_ROTATION_REENCRYPTED_ITEMS
		{
			return Err(bad_request_error(
...
async fn assert_invitation_pending_vault_keys_are_authorized(
	...
	.fetch_one(pool)
	.await
	.map_err(|e| { tracing::error!(error = %e, "Failed to validate pendingVaultKeys vaults"); internal_error("Failed to validate pendingVaultKeys vaults") })?;
...
async fn mark_rotation_records_failed(
	...
	for record in records {
		query("UPDATE vault_key_rotation SET status = 'failed', error_message = $1 WHERE id = $2")
			...
			.execute(pool)
			.await
			.map_err(|e| { tracing::error!(error = %e, "Failed to mark rotation as failed"); internal_error("Failed to mark rotation as failed") })?;
	}
	Ok(())
}
...
async fn apply_team_vault_rotations(
	...
	for vault_rotation in vault_rotations {
		...
		let deleted_rows = query("DELETE FROM vault_key WHERE vault_id = $1 AND user_id = $2")
			...
			.execute(&mut **transaction)
			.await
			.map_err(|e| { tracing::error!(error = %e, "Failed to remove rotated vault access"); internal_error("Failed to remove rotated vault access") })?
			.rows_affected();
		if deleted_rows == 0 {
		...
		query("UPDATE vault_key_rotation SET status = 'completed', completed_at = $1 WHERE id = $2")
			.bind(OffsetDateTime::now_utc())
			.bind(&record.rotation_id)
			.execute(&mut **transaction)
			.await
...
async fn insert_team_member_audit_log(
	...
	query(&query_text)
		...
		.execute(pool)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to record team member audit event"); internal_error("Failed to record team member audit event") })?;
	Ok(())
}
...
async fn create_personal_team_for_user(
	...
	query("UPDATE \"user\" SET team_id = $1, role = 'owner' WHERE id = $2")
		...
		.execute(&mut **transaction)
		.await
		.map_err(|e| { tracing::error!(error = %e, "Failed to reassign team owner"); internal_error("Failed to reassign team owner") })?;
...
mod tests {
	use super::{
		ensure_exact_rotation_vault_set, normalize_pending_vault_keys,
		parse_pending_vault_keys, validate_rotation_vault_inputs, validate_token,
		PendingVaultKeyEntry, RotationMemberKeyInput, RotationReEncryptedItemInput,
		RotationVaultInput, VaultKeyRotationInput,
	};
	...
	fn parse_pending_vault_keys_rejects_invalid_payload() {
		let error = parse_pending_vault_keys(Some("{not-json}"))
			.unwrap_err();
		assert_eq!(error.code, "BAD_REQUEST");
