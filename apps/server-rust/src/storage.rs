...
pub fn create_attachment_key(user_id: &str, item_id: &str, file_name: &str) -> Result<String, StorageError> {
	let safe_name = sanitize_file_name(file_name);
	let upload_id = random_uuid_like();
	let expires_at_ms = chrono::Utc::now().timestamp_millis() + ATTACHMENT_UPLOAD_KEY_TTL_MS;
	let signature = sign_attachment_upload_intent(&format!(
		"{user_id}:{item_id}:{upload_id}:{expires_at_ms}"
	))?;
	Ok(format!(
...
pub fn is_valid_attachment_upload_key(
	...
	let key_user_id = captures.get(1).map(|value| value.as_str()).unwrap_or_default();
	let key_item_id = captures.get(2).map(|value| value.as_str()).unwrap_or_default();
	...
	let upload_id = captures.get(4).map(|value| value.as_str()).unwrap_or_default();
	let signature = captures.get(5).map(|value| value.as_str()).unwrap_or_default();
	...
	let presigned_request = request
		.presigned(
			PresigningConfig::expires_in(Duration::from_secs(
				expires_in_seconds.unwrap_or(300),
			))
...
pub async fn head_object(key: &str) -> Result<Option<StorageObjectHead>, StorageError> {
	...
	match response {
		...
		Err(aws_sdk_s3::error::SdkError::ServiceError(service_error))
			if service_error.err().is_not_found() => Ok(None),
...
fn get_client() -> Result<&'static Client, StorageError> {
	...
	Ok(STORAGE_CLIENT.get().expect("storage client should be initialized"))
}
	...
	match (endpoint, bucket, access_key_id, secret_access_key) {
		(Some(endpoint), Some(bucket), Some(access_key_id), Some(secret_access_key))
			if !endpoint.trim().is_empty()
				...
				Ok(STORAGE_CONFIG.get().expect("storage config should be initialized"))
			}
...
fn get_attachment_upload_signing_secret() -> Result<String, StorageError> {
	match env::var("BITTERY_ATTACHMENT_UPLOAD_SECRET")
