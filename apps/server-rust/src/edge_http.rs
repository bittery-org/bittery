...
use axum::{
	...
	http::{
		header::{
			...
			CONTENT_TYPE, EXPIRES, HeaderName, HeaderValue, ORIGIN, PRAGMA, VARY,
		},
...
pub async fn rpc_request_guard_middleware(
	request: Request<Body>,
	next: Next,
) -> Response {
...
fn parse_cors_origins(raw_value: Option<&str>) -> Result<Vec<String>, String> {
	...
	for origin in raw_value.split(',').map(str::trim).filter(|value| !value.is_empty()) {
		let parsed_origin = assert_valid_origin(origin)?;
		if !seen.insert(parsed_origin.clone()) {
			return Err(format!("CORS_ORIGIN contains a duplicate origin: {parsed_origin}"));
		}
