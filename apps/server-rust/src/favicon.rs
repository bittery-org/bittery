...
pub async fn fetch_and_store_favicon(
	...
	if let Some(existing) = &existing {
		if existing.status == "fetched" && existing.image_data.as_ref().is_some_and(|data| !data.is_empty()) {
			return Ok(true);
		}
...
fn extract_icon_links(html: &str, base_url: &str) -> Vec<String> {
	...
	let rel_regex = Regex::new(r#"\brel\s*=\s*(?:"([^"]*)"|'([^']*)')"#).expect("rel regex should compile");
	let href_regex = Regex::new(r#"\bhref\s*=\s*(?:"([^"]*)"|'([^']*)')"#).expect("href regex should compile");
	...
	for tag_match in link_tag_regex.find_iter(html) {
		...
		let Some(href) = href_capture.get(1).or_else(|| href_capture.get(2)).map(|value| value.as_str().trim()) else {
			continue;
		};
