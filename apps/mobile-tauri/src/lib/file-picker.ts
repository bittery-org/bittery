 ...
 * `tauri-plugin-fs`'s `readFile` reads back the SAF URI it returns. That pair is exercised
 * on `/debug` ("Pick file (dialog+fs)") rather than trusted from the support table, because
 * `dialog` and `fs` are both only partially implemented on mobile.
 */
