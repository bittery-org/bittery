...
export function Favicon({ url, title, category = "login", size = "md", className }: FaviconProps) {
  ...
  const faviconUrl = url && category === "login" ? getFaviconUrl(url, faviconSizeMap[size]) : null;
  ...
  return (
    <div
      className={cn(
        "flex-shrink-0 rounded-lg flex items-center justify-center overflow-hidden border",
        sizeClasses[size],
