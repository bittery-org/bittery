...
Bittery is free and open source software — AGPLv3 for the server, GPLv3 for the clients — and is designed to be self-hosted. You keep complete control over your data: nothing ever leaves your infrastructure, and there is no subscription or license key to run it.
...
Non-loopback HTTP is disabled by default. Supporting it requires an explicit operator capability
and separate confirmation for every affected client account. Recommended deployments keep HTTPS
enabled and pin the server and web containers to one matching release.
