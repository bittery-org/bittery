...
| **Key derivation** | PBKDF2-SHA256 (600k iter) | Derive keys from master password |
...
During login challenge (`startLogin`), the server returns an explicit KDF profile (`schemaVersion`, `algorithm`, and `iterations`) plus an independently validated SRP salt.
...
The entire crypto implementation is public and available for audit at [github.com/bittery-org/bittery](https://github.com/bittery-org/bittery). We believe transparency is fundamental to trust in a security product.
