...
- **Encryption happens locally.** All vault data is encrypted with AES-256-GCM on your device before being sent to our servers.
...
<Callout type="info">
Every encryption operation uses a fresh random IV, ensuring that encrypting the same data twice produces different ciphertext.
</Callout>
...
| Vault items | Yes (AES-256-GCM) | No |
| Vault keys | Yes (AES-256-GCM) | No |
| RSA private key | Yes (AES-256-GCM) | No |
