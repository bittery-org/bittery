...
async fn apply_checkout_session_completed(
    ...
    query(
		"UPDATE team SET billing_plan = COALESCE($1::billing_plan, billing_plan), stripe_customer_id = COALESCE($2, stripe_customer_id), stripe_subscription_id = COALESCE($3, stripe_subscription_id), updated_at = $4 WHERE id = $5",
	)
	.bind(plan)
...
async fn apply_subscription_update(
    ...
    query(
		"UPDATE team SET billing_plan = $1::billing_plan, billing_status = $2::billing_status, stripe_customer_id = COALESCE($3, stripe_customer_id), stripe_subscription_id = $4, stripe_subscription_item_id = $5, stripe_price_id = $6, seats_purchased = $7, current_period_end = $8, cancel_at_period_end = $9, updated_at = $10 WHERE id = $11",
	)
	.bind(billing_plan)
	.bind(billing_status)
...
async fn apply_invoice_status(
    ...
    query("UPDATE team SET billing_status = $1::billing_status, updated_at = $2 WHERE id = $3")
        .bind(billing_status)
        .bind(OffsetDateTime::now_utc())
        .bind(team.id)
        .execute(pool)
...
mod tests {
    ...
    fn verifies_valid_signature() {
        ...
        let mut mac =
            TestHmacSha256::new_from_slice(secret.as_bytes()).expect("test secret should be valid");
        mac.update(signed_payload.as_bytes());
