...
#[openapi(
    ...
    components(schemas(
        ...
        super::dto::DecimalString,
        super::dto::PageCursor,
        super::dto::SyncCursor,
        super::dto::PageRequest,
        super::dto::ProblemDetails,
        super::dto::ProblemFieldError,
        super::error_code::ErrorCode,
        shares::http::ShareToken
    )),
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 500, description = "Metadata lookup failed", body = super::dto::ProblemDetails, content_type = "application/problem+json")
    )
)]
async fn get_meta(State(state): State<AppState>) -> Result<Json<ApiMetadata>, ApiError> {
    let registration = auth::registration_status(&state).await?;
    ...
    let mut router = OpenApiRouter::with_openapi(ApiDoc::openapi()).nest(
        ...
        OpenApiRouter::new().routes(routes!(get_meta)).nest(
            ...
            auth::http::router()
                ...
                .merge(shares::http::router())
                .merge(billing::routes::router())
                .merge(vaults::http::travel_mode::router())
                .merge(teams::audit::routes::router()),
        ),
    );
...
mod tests {
    ...
    fn every_schema_type_name_is_declared_once() {
        ...
        fn collect_rust_sources(directory: &std::path::Path, sources: &mut Vec<PathBuf>) {
        ...
        let mut sources = vec![source_root.join("db/enums.rs")];
        collect_rust_sources(&source_root.join("http"), &mut sources);
        collect_rust_sources(&source_root.join("domains"), &mut sources);
        sources.sort();
    ...
    fn no_two_schema_types_share_a_component_name() {
        ...
        let modules = [
            ...
            (
                "audit",
                crate::domains::teams::audit::routes::router().into_openapi(),
            ),
            ("auth", crate::domains::auth::http::router().into_openapi()),
            (
                "billing",
                crate::domains::billing::routes::router().into_openapi(),
            ),
            (
                "share",
                crate::domains::shares::http::router().into_openapi(),
            ),
            ("sync", crate::domains::sync::http::router().into_openapi()),
            (
                "team",
                crate::domains::teams::routes::router().into_openapi(),
            ),
            (
                "travel_mode",
                crate::domains::vaults::http::travel_mode::router().into_openapi(),
            ),
            (
                "vault",
                crate::domains::vaults::http::router().into_openapi(),
            ),
            (
                "vault_key_rotation",
                crate::domains::vaults::http::rotation::router().into_openapi(),
            ),
        ];
