#[path = "attachment_upload_reservation.rs"]
mod upload_reservation;
pub(crate) use upload_reservation::cleanup_durable_attachment_uploads;
...
async fn insert_registered_attachment(
    transaction: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    input: &CreateAttachmentInput,
    scoped_item: &DbBootstrapItemRow,
    user_id: &str,
    request_client_id: Option<&str>,
    reservation_id: &str,
    storage_size: i32,
...
async fn load_attachment_actor(
    ...
    let actor =
        load_team_billing_entitlement(pool, user_id, "Failed to load attachment entitlements")
            .await?;
    resolve_attachment_actor(actor, deployment_mode, user_id)
}

fn resolve_attachment_actor(
    actor: Option<crate::db::models::DbTeamBillingEntitlementRow>,
    deployment_mode: DeploymentMode,
    user_id: &str,
) -> Result<AttachmentActor, AppError> {
...
pub(super) fn base64_encoded_length(byte_length: i64) -> i64 {
    ((byte_length + 2) / 3) * 4
}

pub(super) fn encrypted_attachment_storage_size(file_size: i32) -> i64 {
    let base64_plaintext_length = base64_encoded_length(i64::from(file_size));
    let ciphertext_length = base64_encoded_length(base64_plaintext_length + 16);
    let iv_length = base64_encoded_length(12);
    40 + ciphertext_length + iv_length + "AES-GCM-AAD-V1".len() as i64
}
