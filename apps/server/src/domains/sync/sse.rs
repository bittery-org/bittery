...
use crate::{
    domains::sessions::control::load_session_revocation,
    domains::sessions::service::VerifiedSession,
    domains::sync::pubsub::SyncNotification,
    domains::sync::{
        generate_sync_connection_id, sse_heartbeat_event, sse_json_event, timestamp_millis,
        SYNC_STREAM_HEARTBEAT_INTERVAL_MS,
    },
    http::error::ApiError,
    shared::connection_registry::{resolve_connection_limit, ConnectionGuard},
    AppState,
};
...
pub(crate) async fn sync_events(
    ...
    if state.connection_registry.is_active() {
        let plan_limit = resolve_connection_limit(
            &state.redis,
            pool,
            &session.user_id,
