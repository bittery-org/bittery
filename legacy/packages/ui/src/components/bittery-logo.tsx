...
/**
 * The Bittery wordmark.
 *
 * Shared from here because it was previously duplicated: once as a component in
 * `apps/marketing/src/components/bittery-logo.tsx` and once inlined into
 * `apps/web/src/components/layout/sidebar.tsx`. Both are the same path data as this.
 *
 * `fill-current` rather than a baked colour, so the mark takes the surrounding text colour and
 * works in both themes without a second asset.
 */
export function BitteryLogo({ className, ...props }: SVGProps<SVGSVGElement>) {
	return (
		<svg
