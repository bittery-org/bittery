...
web, desktop and mobile, direct WASM for the extension, and UniFFI Kotlin over a per-ABI `.so`
(`packages/crypto/android`) for Android's credential provider, which runs in its own process
and cannot reach a WebView. The Rust server uses a direct crate dependency.
...
service worker, and a native library in the Android credential provider. See `SECURITY.md` § "All Crypto in
