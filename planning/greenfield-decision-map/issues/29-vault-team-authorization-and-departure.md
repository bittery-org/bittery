...
recipient Account Fingerprint, **granted role**, and the exact HPKE body. This ticket supplies the
canonical `u8` role enum; the field and payload binding are no longer negotiable.

`CRYPTO-006` makes grants flat, so a Team is an authorization grouping plus a name and never owns any
cryptographic key. That keeps `TEAM-003` and `TEAM-004` true. Departure therefore rotates only the
Vaults the departing member was actually granted.
