...
use super::{
    ...
    persistence_contract::{
        ...
        PreparedLockEpochAdvance, PreparedReplicaWrite, ReplicaHead, ReplicaInstallResult,
        ReplicaPersistenceRequest, ReplicaPersistenceResponse, ReplicaStore, StoredReplicaRow,
    },
...
/// Generated bytes have a canonical order; the shared persistence contract does not. Writes to
/// independent row keys commute. Stable sorting preserves the original order of multiple effects
/// on one key (for example Delete then Put), and never touches Account/revision/epoch guards.
fn canonicalize_request(mut request: ReplicaPersistenceRequest) -> ReplicaPersistenceRequest {
    let writes = match &mut request {
    ...
    writes.sort_by(|left, right| write_key(left).cmp(&write_key(right)));
    request
}

fn write_key(write: &PreparedReplicaWrite) -> (u8, &str, &str) {
    match write {
        PreparedReplicaWrite::Put { row } => row_key(row),
        PreparedReplicaWrite::Delete { store, key } => {
            (store_order(*store), key.account_id.as_str(), &key.record_id)
        }
    }
}
...
fn row_key(row: &StoredReplicaRow) -> (u8, &str, &str) {
    (
        store_order(row.store),
        row.key.account_id.as_str(),
        &row.key.record_id,
    )
}

fn store_order(store: ReplicaStore) -> u8 {
    match store {
...
/// One frozen ordered Import batch, fingerprinted from its own bytes.
fn import_operation(
    operation_id: &str,
    indices: &[usize],
...
fn import_reconciliation_history() -> Result<History, RuntimeError> {
    ...
    let mut history = HistoryBuilder::new(
        ...
        &[
            "five-category batch authority under one applied receipt",
            "multi-Item acceptance and reconciliation commits",
    ...
    let mut revision = 2;
    for (category, indices) in [("first", &[0, 1, 2, 3, 4][..]), ("second", &[0, 3][..])] {
        let applied = import_operation(&format!("operation-import-{category}"), indices)?;
        history.commit_plan(
            ...
            GuardedCommitPlan::new(
                ...
                vec![PlanMutation::ReconcileImportItems {
                    outcome: ObservedOutcome {
                        ...
                        result: OperationOutcomeResult::ImportApplied {
                            vault_id: "vault-1".into(),
                            imported_count: indices.len() as u16,
                        },
                    },
                    items: indices.iter().map(|&index| import_authority(account_id, &applied.operation_id, index)).collect(),
                }],
            ),
        )?;
        revision += 1;
    ...
    let rejected = import_operation("operation-import-rejected", &[1, 2])?;
    history.commit_plan(
...
fn five_category_authority_history() -> Result<History, RuntimeError> {
    ...
    plan.items = categories
        .into_iter()
        .map(|(item_id, category)| {
            let mut item = authority_item("account-categories", item_id, 1);
            item.category = category;
            item
        })
        .collect();
    history.stage_bootstrap("stage five-category Item page", plan)?;
    history.promote_bootstrap(
...
#[cfg(test)]
#[test]
fn generator_canonicalizes_independent_bootstrap_maps_and_insertion_permutations() {
    let history = five_category_authority_history().unwrap();
    let page_index = history
        .steps
        .iter()
        .position(|step| step.label == "stage five-category Item page")
        .unwrap();
    let load = |step: &Step| {
        let state = &step.expected_loaded_state[0];
        let ReplicaPersistenceResponse::Loaded { head, rows } = &state.response else {
            panic!("loaded fixture")
        };
        reconstruct_snapshot(&state.account_id, head.clone(), rows.clone())
            .unwrap()
            .unwrap()
    };
    let current = load(&history.steps[page_index - 1]);
    let template = load(&history.steps[page_index]);
    let mut entries = template
        ...
        .collect::<Vec<_>>();
    entries.sort_by(|left, right| left.0 .1.cmp(&right.0 .1));
    assert_eq!(entries.len(), 5);
    let mut expected = None;
    for reverse in [false, true] {
        for rotation in 0..16 {
            // A fresh HashMap has an independently initialized RandomState. Cloning one map or
            // serializing it twice would miss the cross-process nondeterminism this reproduces.
            // Both snapshots come from an actual Domain Bootstrap page, not invented row state.
            let mut next = template.clone();
            next.bootstrap.items = std::collections::HashMap::new();
            let mut indices = (0..entries.len()).collect::<Vec<_>>();
            indices.rotate_left(rotation % entries.len());
            if reverse {
                indices.reverse();
            }
            for index in indices {
                let (key, item) = &entries[index];
                next.bootstrap.items.insert(key.clone(), item.clone());
            }
            let prepared = prepare_bootstrap_commit(current.clone(), next, false).unwrap();
            let request = ReplicaPersistenceRequest::Commit {
                prepared: prepared.wire,
            };
            ...
            if let Some(expected) = &expected {
                assert_eq!(
                    &bytes, expected,
                    "equivalent fresh maps must emit identical generated requests"
                );
            } else {
                expected = Some(bytes);
            }
...
#[cfg(test)]
#[test]
fn generator_sort_preserves_same_key_effect_order_and_all_guards() {
    use super::persistence_contract::{PreparedReplicaCommit, ReplicaRowKey};
    let account_id = AccountId::from("account-write-order");
    let put = |store, key: &str, payload: &str| PreparedReplicaWrite::Put {
    ...
    let delete = |store, key: &str| PreparedReplicaWrite::Delete {
        store,
        key: ReplicaRowKey {
            account_id: account_id.clone(),
            record_id: key.into(),
    ...
    let writes = vec![
        put(ReplicaStore::AuthorityItems, "b", "first"),
        delete(ReplicaStore::AuthorityItems, "a"),
        put(ReplicaStore::Operations, "operation", "accepted"),
        delete(ReplicaStore::AuthorityItems, "b"),
        put(ReplicaStore::AuthorityItems, "b", "final"),
        put(ReplicaStore::AuthorityItems, "a", "replacement"),
    ];
    let mut install = prepare_install(
        None,
        account_id.clone(),
        "user-write-order".into(),
        incarnation(account_id.as_str(), "first"),
    )
    .unwrap();
    install.writes = writes.clone();
    let commit = PreparedReplicaCommit {
        expected: ExpectedReplicaHead {
            account_id: account_id.clone(),
            user_id: install.next_head.user_id.clone(),
            incarnation: install.next_head.incarnation.clone(),
            replica_revision: 41,
            lock_epoch: 7,
        },
        next_head: ReplicaHead {
            replica_revision: 42,
            lock_epoch: 7,
            ..install.next_head.clone()
        },
        writes,
    };
    for original in [
        ReplicaPersistenceRequest::Install { prepared: install },
        ReplicaPersistenceRequest::Commit { prepared: commit },
    ] {
        let mut canonical = canonicalize_request(original.clone());
        let (original_writes, canonical_writes) = match (&original, &mut canonical) {
            (
                ReplicaPersistenceRequest::Install { prepared: left },
                ReplicaPersistenceRequest::Install { prepared: right },
            ) => (&left.writes, &mut right.writes),
            (
                ReplicaPersistenceRequest::Commit { prepared: left },
                ReplicaPersistenceRequest::Commit { prepared: right },
            ) => (&left.writes, &mut right.writes),
            _ => panic!("canonicalization changed the request kind"),
        };
        assert_ne!(
            original_writes, canonical_writes,
            "the fixture must exercise actual reordering"
        );
        assert!(canonical_writes
            .windows(2)
            .all(|pair| write_key(&pair[0]) <= write_key(&pair[1])));
        let mut expected = apply_prepared_writes_to_rows(Vec::new(), original_writes);
        let mut actual = apply_prepared_writes_to_rows(Vec::new(), canonical_writes);
        expected.sort_by(|left, right| row_key(left).cmp(&row_key(right)));
        actual.sort_by(|left, right| row_key(left).cmp(&row_key(right)));
        assert_eq!(
            actual, expected,
