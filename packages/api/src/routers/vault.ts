  ...
  search: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const matchingItems = allItems
        .filter((item) => {
          const overview = item.overview as { title?: string; url?: string; username?: string };
          const titleMatch = overview.title?.toLowerCase().includes(searchLower);
          const urlMatch = overview.url?.toLowerCase().includes(searchLower);
          const usernameMatch = overview.username?.toLowerCase().includes(searchLower);
        ...
        .map((item) => {
          ...
          return {
            ...
            overview: item.overview as { title: string; url?: string; username?: string },
          };
        });
