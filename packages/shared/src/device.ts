 ...
 */
export function getDesktopDeviceInfo(osName: string, osVersion?: string): DeviceInfo {
  return {
 ...
 */
export function getExtensionDeviceInfo(browserName: string, browserVersion?: string): DeviceInfo {
  return {
...
export function formatDeviceDisplay(device: {
  ...
  const parts: string[] = [];
  if (device.osName) {
    parts.push(device.osVersion ? `${device.osName} ${device.osVersion}` : device.osName);
  }
  if (device.browserName && device.browserVersion) {
    parts.push(`${device.browserName} ${device.browserVersion}`);
  }

  const subtitle = parts.length > 0 ? parts.join(" - ") : (device.platform ?? "Unknown");
...
export function formatLastActive(date: Date | string): string {
  ...
  return lastActive.toLocaleDateString(undefined, {
    month: "short",
    day: "numeric",
    year: lastActive.getFullYear() !== now.getFullYear() ? "numeric" : undefined,
  });
}
